...
def window_creation(df, size_window, label_column, label_2_column, timestamp_colum):
    ...
    for i in range(size_window, len(df) - size_window, 8):
        ...
        label_data.append(
            to_label(label_df.loc[i + size_window - 8:i + size_window - 1]))  # Assign the label of the target day
...
def linker(
    ...
    for root, dirs, files in os.walk(time_data):
        for fil in files:
            ...
            if fil.endswith('.csv'):
                ...
                my_features = my_features[features].fillna(fillNA).replace(
                    {'mq': fillNA, 'No_Crisis': 0, 'Crisis': 1, 'Ecological': 1, 'Sudden': 2})
                    ...
                    if text_line['date'] in set_of_date:
                        ...
                        if text_line['Related station'] != text_line['Related station']:  # False if NaN
                            ...
                            if text_line['CAT2'] == 'Message-NonUtilisable':
                                ...
                                if text_line['type_crisis'] == 'Flood' or text_line['type_crisis'] == 'Hurricane' or \
                                        text_line['type_crisis'] == 'Storms':
    ...
    df_return = pd.DataFrame(
        {'Date': list_of_date, 'Text': list_of_text, 'Crisis_Type': list_of_crisis_type, 'Window': list_of_window,
         'label': list_of_label})
    return df_return