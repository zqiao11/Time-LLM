...
class Model(nn.Module):
    ...
    def __init__(self, configs, n_classes, n_vars):
        ...
        # ToDo: How TimeLLM do patch embedding?
        self.patch_embedding = MultiVariatePatchEmbedding(configs.d_model, n_vars, self.patch_len, self.stride, configs.dropout)
        ...
        self.output_projection = nn.Linear(in_features=self.d_ff, out_features=n_classes)

        self.normalize_layers = Normalize(configs.enc_in, affine=False)

    def forward(self, x_text, x_ts):
        logits = self.classify(x_text, x_ts)
        return logits

    def classify(self, x_text, x_ts):
        """
